...
func (skyc *Skycore) get_dockerimage_shocknode_attributes(node_id string) (image_repository string, image_tag string, image_id string, err error) {
	...
	//attr_json, err := json.Marshal(node_response) // ugly hack to get attributes (type: map[string]interface{}) into struct
	//fmt.Sprintf("marshal: %s", attr_json)
	//os.Stdout.Write(attr_json)
	...
	if request_tag != "" {
		...
		if err != nil {
			...
			if strings.TrimPrefix(some_old_image_obj.ID, "sha256:") != image_id {
				...
				err = skyc.Docker_client.RemoveImageExtended(some_old_image_obj.ID, docker.RemoveImageOptions{Force: true, NoPrune: false})
				if err != nil {
